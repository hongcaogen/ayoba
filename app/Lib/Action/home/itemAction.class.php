...
class itemAction extends frontendAction {
    ...
    public function index() {
        ...
        !$id && $this->_404();
        $item_mod = M('item');
        $item = $item_mod->field('id,cate_id,title,uid,uname,intro,price,url,is_localimg,likes,comments,tag_cache,seo_title,seo_keys,seo_desc,add_time')->where(array('id' => $id, 'status' => 1))->find();
        !$item && $this->_404();
        //来源
        ...
        //属性
        $item_attr = M('item_attr')->where(array('item_id'=>$id))->select();
        
        //可能还喜欢
        $item_tag_mod = M('item_tag');
        $db_pre = C('DB_PREFIX');
        $item_tag_table = $db_pre . 'item_tag';
        $maylike_list = array_slice($item['tag_list'], 0, 1, true);
        foreach ($maylike_list as $key => $val) {
            $maylike_list[$key] = array('name' => $val);
            $maylike_list[$key]['list'] = $item_tag_mod->field('i.id,i.title,i.img,i.intro,i.is_localimg,' . $item_tag_table . '.tag_id')->where(array($item_tag_table . '.tag_id' => $key, 'i.status'=>1, 'i.id' => array('neq', $id)))->join($db_pre . 'item i ON i.id = ' . $item_tag_table . '.item_id')->order('i.id DESC')->limit(9)->select();
        }
        
        $this->waterfall(array('cate_id'=>$item['cate_id']), 'id DESC', '', 1);
    ...
    public function publish_item() {
        ...
        //var_dump($item['key_id']);exit;
       // !$item['key_id'] && $this->ajaxReturn(0, L('publish_item_failed'));
        $album_id = $this->_post('album_id', 'intval', 0);
        $ac_id = $this->_post('ac_id', 'intval', 0);
        $item['item']['intro'] = $this->_post('intro', 'trim');
        $item['item']['info'] = Input::deleteHtmlTags($item['info']);
        $item['item']['uid'] = $this->visitor->info['id'];
        $item['item']['uname'] = $this->visitor->info['username'];
        $item['item']['status'] = C('pin_item_check') ? 0 : 1;
        //添加商品
        $item_mod = D('item');
        $result = $item_mod->publish($item, $album_id, $ac_id);
        if ($result) {
            //发布商品钩子
            $tag_arg = array('uid' => $item['item']['uid'], 'uname' => $item['item']['uname'], 'action' => 'pubitem');
