...
class shop_cateAction extends backendAction {
    ...
    public function index() {
        ...
        foreach($result as $r) {
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('shop_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_article_cate').'" data-id="add" data-width="500" data-height="360">'.L('add_article_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('shop_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="360">'.L('edit').'</a> |
                                <a href="javascript:;" data-acttype="ajax" class="J_confirmurl" data-uri="'.U('shop_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
        ...
        $big_menu = array(
            'title' => L('add_shop_cate'),
            'iframe' => U('shop_cate/add'),
            'id' => 'add',
            'width' => '500',
            'height' => '360'
        );
    ...
    protected function _before_insert($data = '') {
        ...
        if($this->_mod->name_exists($data['name'], $data['pid'])){
            $this->ajaxReturn(0, L('shop_cate_already_exists'));
        }
        //生成spid
    ...
    public function ajax_upload_img() {
        ...
        if (!empty($_FILES['img']['name'])) {
            $result = $this->_upload($_FILES['img'], 'shop_cate', array('width'=>'80', 'height'=>'80'));
            if ($result['error']) {
	...
	public function ajax_get_artilcechilds() {
        $id = $this->_get('id', 'intval');
        $id?$id:0;
        $return = $this->_mod->field('id,name')->where(array('type'=>0,'pid'=>$id))->select();
        if ($return) {
            $this->ajaxReturn(1, L('operation_success'), $return);
        } else {
            $this->ajaxReturn(0, L('operation_failure'));
