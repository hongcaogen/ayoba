...
class collect_alimamaAction extends backendAction {
    ...
    public function publish() {
        if (IS_POST) {
            ...
            //var_dump($taobaoke_item_list);exit;
    ...
    private function _get_list($map, $p) {
        ...
        foreach ($resp_items['taobaoke_item'] as $val) {
            ...
            //店铺信息
	        $req = $tb_top->load_api('ShopGetRequest');
		    
	        $req->setFields('sid,cid,nick,desc,bulletin,pic_path,created,modified');
		    $req->setNick($val['nick']);
		    $resp = $tb_top->execute($req);
		    $shop = (array) $resp->shop;
		    
		    $val['shop']['name'] = $shop['nick'];
		    $val['shop']['img'] = 'http://logo.taobao.com/shop-logo'.$shop['pic_path'];
		    $val['shop']['url'] = 'http://shop'.$shop['sid'].'.taobao.com';
		    $val['shop']['taoke_url'] = Url::replace($taoke_url, array('spm' => '2014.21069764.' . $api_config['app_key'] . '.0'));
		    //$val['shop']['add_time'] = time();
        ...
        foreach ($resp_item_list as $val) {
            ...
            //属性
            $item_props = explode(';', $val['props_name']);
            foreach ($item_props as $_prop) {
                $_prop_arr = explode(':', $_prop);
                if (isset($props[$_prop_arr[0]])) {
                    $props[$_prop_arr[0]]['value'] .= ' '.$_prop_arr[3];
                } else {
                    $props[$_prop_arr[0]] = array('name'=>$_prop_arr[2], 'value'=>$_prop_arr[3]);
                }
            }
            $taobaoke_item_list[$val['num_iid']]['props'] = $props;
