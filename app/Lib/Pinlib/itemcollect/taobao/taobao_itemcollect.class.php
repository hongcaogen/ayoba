    ...
    public function fetch($url) {
        ...
        if (isset($resp->taobaoke_item_details)) {
            ...
            if ($taoke['click_url']) {
                ...
                $taoke_url = $taoke['shop_click_url'];
            }
        }
        ...
        $req->setNick($item['nick']);
        $resp = $tb_top->execute($req);
        $shop = (array) $resp->shop;
        //$result['shop']['key_id'] = 'taobao_shop_' . $shop['sid'];
        $result['shop']['name'] = $shop['nick'];
        $result['shop']['img'] = 'http://logo.taobao.com/shop-logo'.$shop['pic_path'];
        $result['shop']['url'] = 'http://shop'.$shop['sid'].'.taobao.com';
        $result['shop']['taoke_url'] = Url::replace($taoke_url, array('spm' => '2014.21069764.' . $api_config['app_key'] . '.0'));
        //$result['shop']['add_time'] = time();
        return $result;
    }
